...
class LeaderReplicaExchangeRunner:
    ...
    def run(
        ...
        while self._step <= self._max_steps:
            ...
            # Safety assertion: states must be concrete State objects, not nested lists
            for _idx, _s in enumerate(leader_states):
                if isinstance(_s, list):
                    detail_types = [type(x).__name__ for x in _s[:4]]
                    raise TypeError(
                        f"[state-shape-error] leader: leader_states[{_idx}] is a list (len={len(_s)}) instead of State; sample types={detail_types}."
                    )
                if not hasattr(_s, "alpha"):
                    raise AttributeError(
                        f"[state-shape-error] leader: leader_states[{_idx}] object of type {type(_s).__name__} missing 'alpha' attribute."
                    )
            ...
            for i, (state, alpha) in enumerate(zip(leader_states, my_alphas)):
                try:
                    ...
                    sys.stderr.write(
                        f"[leader-debug] AttributeError setting alpha at step={self._step} i={i} type={type(state).__name__} is_list={isinstance(state,list)} has_alpha={hasattr(state,'alpha')} alpha_value={alpha} len_leader_states={len(leader_states)}\n"
                    )
                    if isinstance(state, list):
                        sys.stderr.write(f"[leader-debug] offending element is a list: inner_types={[type(x).__name__ for x in state[:4]]} inner_len={len(state)}\n")
                    sys.stderr.write(f"[leader-debug] traceback:\n{tb}\n")
                    sys.stderr.flush()
                    raise
